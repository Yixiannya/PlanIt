...
export default function GroupUser({route}) {
  ...
  const handleDelete = async (gp, id) => {
      const id_array = {
          id: gp,
          userId: user._id,
  ...
  const handleAdminAdding = async (gp, id) => {
      const id_array_add = {
          id: gp,
          userId: user._id,
    ...
    const handleAdminDeleting = async (gp, id) => {
        const id_array = {
            id: gp,
            userId: user._id,
  ...
  const Item = ({ item }) => (
    <View className="flex-1 flex-row items-center justify-center m-2 p-5 bg-orange-400 rounded-2xl">
        ...
        {deleting && (
            <TouchableOpacity onPress = {() => handleDelete(Group._id, item)}>
                <Image source={require('../assets/delete.png')} className="w-12 h-12" />
            </TouchableOpacity>
        )}
        {adminAdding && (
            <TouchableOpacity onPress = {() => handleAdminAdding(Group._id, item)}>
  ...
  const ItemAdmin = ({ item }) => (
      <View className="flex-1 flex-row items-center justify-center m-2 p-5 bg-orange-400 rounded-2xl">
          <Text className="text-xl font-bold">{item.name}</Text>
          {adminDeleting && (
              <TouchableOpacity onPress = {() => handleAdminDeleting(Group._id, item)}>
                  <Image source={require('../assets/delete.png')} className="w-12 h-12" />
              </TouchableOpacity>
          )}
      </View>
